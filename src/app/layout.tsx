      ...
      <nav style={{ background: 'var(--card)', borderBottom: '1px solid var(--border)', position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '60px' }}>
          ...
          <Link href="/portal" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', textDecoration: 'none', flexShrink: 0 }}>
            ...
            {!isMobile && (
              <span style={{ fontFamily: 'JerseyM54, Georgia, serif', fontSize: '1rem', color: 'var(--gold)', letterSpacing: '0.03em', whiteSpace: 'nowrap' }}>
          ...
          {/* Desktop nav */}
          {!isMobile && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.1rem', flex: 1, justifyContent: 'center', padding: '0 1rem' }}>
              {navItems.map(item => {
                const active = pathname === item.href
                return (
                  <Link key={item.href} href={item.href} style={{
                    padding: '0.4rem 0.6rem', borderRadius: '0.625rem', fontSize: '0.75rem', fontWeight: 600,
              ...
              {profile?.is_admin && (
                <Link href="/portal/admin" style={{
                  padding: '0.4rem 0.6rem', borderRadius: '0.625rem', fontSize: '0.75rem', fontWeight: 700,
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexShrink: 0 }}>
            ...
            {isMobile && (
              <button onClick={() => setMenuOpen(!menuOpen)} style={{ background: 'none', border: 'none', color: 'var(--cream)', fontSize: '1.5rem', cursor: 'pointer', padding: '0.25rem' }}>
                {menuOpen ? '✕' : '☰'}
              </button>
            )}
          </div>
